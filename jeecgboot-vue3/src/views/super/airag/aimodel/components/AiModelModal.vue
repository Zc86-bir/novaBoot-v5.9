        ...
        <a-tab-pane :key="1">
          ...
          <div class="model-content">
            <BasicForm @register="registerForm">
              ...
              <template #protocol="{ model, field }">
                <a-select v-model:value="model[field]" @change="handleProtocolChange" :bordered="false">
...
<script lang="ts">
  ...
  export default {
    ...
    setup(props, { emit }) {
        ...
        if (data.id) {
          ...
          if (values) {
            if(values.result.credential){
              ...
              //update-begin---author:Zc86-bir ---date:2026-05-03  for：自定义供应商额外凭证字段-----------
              if (credential.protocol) {
                values.result.protocol = credential.protocol;
              }
              //update-end---author:Zc86-bir ---date:2026-05-03  for：自定义供应商额外凭证字段-----------
            }
      ...
      async function save() {
        try {
          ...
          if (values.provider === 'CUSTOM') {
            credential.protocol = values.protocol || 'openai';
          }
          //update-end---author:Zc86-bir ---date:2026-05-03  for：自定义供应商额外凭证字段-----------
          let params = {};
       ...
       * @param value
       */
      async function handleProtocolChange(value) {
        // OpenAI协议默认加/v1，Anthropic不需要
        if (value === 'openai') {
          setFieldsValue({ 'baseUrl': 'https://api.example.com/v1' })
        } else {
          setFieldsValue({ 'baseUrl': 'https://api.anthropic.com' })
        }
      }
