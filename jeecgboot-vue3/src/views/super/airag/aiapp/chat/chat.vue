...
<script setup lang="ts">
  ...
  async function onConversation(userMessage) {
    ...
    // 检查是否需要设置额外参数（只在有必填流程输入时阻止发送）
    if (props.hasExtraFlowInputs && appData.value?.flowInputs) {
      const requiredInputs = appData.value.flowInputs.filter(input => input.required);
  ...
  async function sendMessage(message, options) {
    // 获取用户选择的模型ID
    const selectedModelId = props.conversationSettings?.selectedModelId;

    let param = {};
    if (!props.type && props.type != 'view') {
      // 如果用户选择了模型，创建一个修改后的appData对象
      const appDataToSend = selectedModelId
        ? { ...appData.value, modelId: selectedModelId }
        : appData.value;
  ...
  watch(
    ...
    (val) => {
      if (val && Object.keys(val).length > 0) {
        // 只有当模型支持对应功能时才应用用户设置
        if (showThink.value && val.enableThink !== undefined) {
          enableThink.value = val.enableThink;
        }
        if (showWebSearch.value && val.enableSearch !== undefined) {
          enableSearch.value = val.enableSearch;
        }
        if (showDraw.value && val.enableDraw !== undefined) {
          enableDraw.value = val.enableDraw;
        }
        // 保存到localStorage
        saveAiSettings();
      }
  ...
  onMounted(() => {
    ...
    if (savedSettings) {
      try {
        ...
        // 只有当模型支持对应功能时才应用用户设置
        if (showThink.value) {
          enableThink.value = parsed.enableThink || false;
        }
        if (showWebSearch.value) {
          enableSearch.value = parsed.enableSearch || false;
        }
        if (showDraw.value) {
          enableDraw.value = parsed.enableDraw || false;
        }
      } catch (e) {
