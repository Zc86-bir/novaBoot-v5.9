<template>
  ...
  >
    <div class="settings-content">
      <!-- AI助手功能配置 -->
      <div class="section-title">AI助手功能配置</div>
      <a-form :model="formData" layout="vertical">
        <a-form-item label="选择模型">
          <a-select
            v-model:value="aiSettings.selectedModelId"
            placeholder="请选择AI模型"
            :options="modelOptions"
            :loading="modelLoading"
            @dropdownVisibleChange="handleModelDropdownChange"
          />
          <span class="setting-desc">选择用于对话的AI模型</span>
        </a-form-item>
        <a-form-item label="深度思考">
          <a-switch v-model:checked="aiSettings.enableThink" />
          <span class="setting-desc">开启后AI将进行更深入的思考分析</span>
        </a-form-item>
        <a-form-item label="联网搜索">
          <a-switch v-model:checked="aiSettings.enableSearch" />
          <span class="setting-desc">开启后AI可以搜索网络获取最新信息</span>
        </a-form-item>
        <a-form-item label="图像生成">
          <a-switch v-model:checked="aiSettings.enableDraw" />
          <span class="setting-desc">开启后AI可以根据描述生成图像</span>
        </a-form-item>
      </a-form>
      ...
      <template v-if="flowInputs.length > 0">
        <a-divider />
        <div class="section-title">流程参数配置</div>
        <a-form :model="formData" layout="vertical">
          <a-form-item
...
<script setup lang="ts">
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      if (res.success && res.result?.records) {
        modelOptions.value = res.result.records
          .filter(item => item.activateFlag === 1 && item.modelType === 'LLM')
          .map(item => ({
            label: item.name,
            value: item.id,
          }));
  ...
  // 模型下拉框展开时加载模型列表
  const handleModelDropdownChange = (open: boolean) => {
    if (open && modelOptions.value.length === 0) {
      fetchModels();
    }
  };
  ...
  const open = () => {
    ...
    if (savedSettings) {
      try {
        ...
        aiSettings.value = {
          enableThink: parsed.enableThink || false,
          enableSearch: parsed.enableSearch || false,
          enableDraw: parsed.enableDraw || false,
          selectedModelId: parsed.selectedModelId || undefined,
        };
      } catch (e) {
  ...
  const handleOk = async () => {
    ...
    localStorage.setItem('aiAssistantSettings', JSON.stringify({
      enableThink: aiSettings.value.enableThink,
      enableSearch: aiSettings.value.enableSearch,
      enableDraw: aiSettings.value.enableDraw,
      selectedModelId: aiSettings.value.selectedModelId,
    }));
