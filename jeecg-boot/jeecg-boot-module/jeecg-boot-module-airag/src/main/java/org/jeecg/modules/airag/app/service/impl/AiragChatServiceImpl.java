...
public class AiragChatServiceImpl implements IAiragChatService {
            ...
            if (topicId.equals(history.getTopicId())) {
                ...
                switch (history.getRole()) {
                        ...
                        if (oConvertUtils.isObjectNotEmpty(history.getToolExecutionRequests())) {
                            ...
                            List<ToolExecutionRequest> toolRequests = history.getToolExecutionRequests().stream()
                                    .map(toolReq -> ToolExecutionRequest.builder()
                                            ...
                                            .build())
                                    .collect(Collectors.toList());
                            // DeepSeek 推理模型要求历史消息必须携带 thinking 字段
                            chatMessage = AiMessage.builder()
                                    .text(aiContent)
                                    .thinking("...")  // 占位符，让 LangChain4j 把 reasoning_content 带上
                    ...
                    case AiragConsts.MESSAGE_ROLE_TOOL:
                        // 重建工具执行结果消息
                        //update-begin---author:Claude ---date:2026-05-09---for:【MCP工具修复】从JSON解析工具ID和名称-----------
                        String toolId;
                        String toolName = "unknown";
                        String toolContent = history.getContent();

                        // 尝试解析JSON格式的content（新格式）
                        try {
                            JSONObject toolInfo = JSONObject.parseObject(toolContent);
                            toolId = toolInfo.getString("id");
                            toolName = toolInfo.getString("name");
                            if (oConvertUtils.isEmpty(toolName)) {
                                toolName = "unknown";
                            }
                        } catch (Exception e) {
                            // 解析失败，说明是旧格式（直接保存的toolId）
                            toolId = toolContent;

                            // 在历史消息中查找对应的工具请求（往前找，因为消息历史是倒序遍历的）
                            for (int j = i - 1; j >= 0; j--) {
                                MessageHistory aiHistory = messagesHistory.get(j);
                                if (AiragConsts.MESSAGE_ROLE_AI.equals(aiHistory.getRole())) {
                                    List<MessageHistory.ToolExecutionRequestHistory> toolReqs = aiHistory.getToolExecutionRequests();
                                    if (oConvertUtils.isObjectNotEmpty(toolReqs)) {
                                        for (MessageHistory.ToolExecutionRequestHistory toolReq : toolReqs) {
                                            if (toolReq != null && toolReq.getId().equals(toolId)) {
                                                toolName = toolReq.getName();
                                                break;
                                            }
                                        }
                                    }
                        ...
                        //update-end---author:Claude ---date:2026-05-09---for：【MCP工具修复】从JSON解析工具ID和名称-----------
    ...
    private void appendMessage(List<ChatMessage> messages, ChatMessage message, ChatConversation chatConversation, String topicId, List<String> files, String saveContent) {
        ...
        if (message instanceof UserMessage) {
            ...
            //update-begin---author:Claude ---date:2026-05-09---for:【MCP工具修复】同时保存工具ID和名称-----------
            // 将工具ID和名称保存为JSON格式，方便重建时解析
            JSONObject toolInfo = new JSONObject();
            toolInfo.put("id", toolMessage.id());
            toolInfo.put("name", toolMessage.toolName() != null ? toolMessage.toolName() : "unknown");
            historyMessage.setContent(toolInfo.toJSONString());
            //update-end---author:Claude ---date:2026-05-09---for：【MCP工具修复】同时保存工具ID和名称-----------
    ...
    private void sendWithAppChat(String requestId, List<ChatMessage> messages, ChatConversation chatConversation, String topicId, ChatSendParams sendParams, String flowId, String memoryId) {
        ...
        // update-begin---author:Claude ---date:2026-05-09---for:【AI路况分析】自动注入地图工具专业提示词-----------
        // 如果AI应用使用了地图类MCP工具，追加专业提示词
        String pluginConfig = aiApp.getPlugins();
        if (oConvertUtils.isNotEmpty(pluginConfig) && pluginConfig.contains("amap")) {
            // 检查是否已包含地图提示词，避免重复
            boolean hasTrafficPrompt = messages.stream()
                .filter(msg -> msg instanceof SystemMessage)
                .map(msg -> (SystemMessage) msg)
                .anyMatch(sysMsg -> sysMsg.text().contains("maps_geo") || sysMsg.text().contains("路况分析"));

            if (!hasTrafficPrompt) {
                appendMessage(messages, new SystemMessage(Prompts.TRAFFIC_ANALYST_PROMPT), chatConversation, topicId);
                log.info("[AI应用]自动注入路况分析专业提示词，应用ID：{}", aiApp.getId());
            }

            // 添加工具调用状态跟踪，防止循环调用
            String toolCallStatus = buildToolCallStatusMessage(messages);
            if (oConvertUtils.isNotEmpty(toolCallStatus)) {
                appendMessage(messages, new SystemMessage(toolCallStatus), chatConversation, topicId);
                log.info("[AI应用]追加工具调用状态提示词：{}", toolCallStatus.substring(0, Math.min(100, toolCallStatus.length())));
            }
        }
        // update-end---author:Claude ---date:2026-05-09---for：【AI路况分析】自动注入地图工具专业提示词-----------
    ...
    private void sendWithDefault(String requestId, ChatConversation chatConversation, String topicId, String modelId, List<ChatMessage> messages, AIChatParams aiChatParams, String sessionType) {
        ...
        //update-begin---author:Claude ---date:2026-05-09---for:【免费搜索】优先使用 SearXNG 免费搜索，无配置时回退到 BraveSearch-----------
        // 联网检索工具：前端 enableSearch=true 时注入
        if (Boolean.TRUE.equals(aiChatParams.getEnableSearch())) {
            ...
            if (searxngConfig != null && oConvertUtils.isNotEmpty(searxngConfig.getBaseUrl())) {
                org.jeecg.modules.airag.app.handler.SearXNGSearchToolBuilder.SearXNGSearchConfig config =
                        new org.jeecg.modules.airag.app.handler.SearXNGSearchToolBuilder.SearXNGSearchConfig();
                config.setBaseUrl(searxngConfig.getBaseUrl());
                config.setApiKey(searxngConfig.getApiKey());
                config.setCount(searxngConfig.getCount());
                config.setTimeout(searxngConfig.getTimeout());
                config.setLanguage(searxngConfig.getLanguage());
                searchTools = org.jeecg.modules.airag.app.handler.SearXNGSearchToolBuilder.buildTools(config);
            ...
            AiRagConfigBean.OpenCLIConfig opencliConfig = aiRagConfigBean.getOpencli();
            if (opencliConfig != null && opencliConfig.isEnabled()) {
                org.jeecg.modules.airag.app.handler.OpenCLIToolBuilder.OpenCLIConfig config =
                        new org.jeecg.modules.airag.app.handler.OpenCLIToolBuilder.OpenCLIConfig();
                config.setEnabled(opencliConfig.isEnabled());
                config.setCommand(opencliConfig.getCommand());
                config.setTimeout(opencliConfig.getTimeout());
                config.setDefaultLimit(opencliConfig.getDefaultLimit());
                Map<ToolSpecification, ToolExecutor> opencliTools = org.jeecg.modules.airag.app.handler.OpenCLIToolBuilder.buildTools(config);
                if (!opencliTools.isEmpty()) {
                    searchTools.putAll(opencliTools);
                    log.info("[AI应用]已注入 OpenCLI 浏览器自动化工具");
                }
            }
        ...
        try {
            ...
            //update-begin---author:Claude ---date:2026-05-09---for:【DeepSeek推理模型】修复thinking字段传递问题-----------
            // DeepSeek 推理模型在多轮工具调用中需要正确传递 reasoning_content 字段
            // LangChain4j 1.12.2 存在 streaming tool call 丢失问题（PR #4890 在 1.14.0 修复）
            // 临时方案：保持 thinking 启用，但确保消息历史中正确重建 thinking 字段
            if (org.jeecg.modules.airag.llm.consts.LLMConsts.isDeepSeekThinkingModel(
                    aiChatParams.getModelName())) {
                log.info("[AI应用]DeepSeek 推理模型启用 thinking 模式，确保消息历史正确重建 reasoning_content");
                // 启用 thinking 的接收和发送
                if (null == aiChatParams.getReturnThinking()) {
                    aiChatParams.setReturnThinking(true);
                }
                if (null == aiChatParams.getSendThinking()) {
                    aiChatParams.setSendThinking(true);
                }
                // 不设置 noThinking，允许模型使用推理能力
            }
            //update-end---author:Claude ---date:2026-05-09---for：【DeepSeek推理模型】修复thinking字段传递问题-----------
        ...
        chatStream.onPartialResponse((String resMessage) -> {
            ...
            // ⚠️ 必须在 onToolExecuted 之前保存，以确保消息顺序正确
            // LangChain4j 要求：ToolExecutionResultMessage 必须在 AiMessage(tool_calls) 之后
            ...
            if (aiMessage != null && oConvertUtils.isObjectNotEmpty(aiMessage.toolExecutionRequests())) {
                ...
                // DeepSeek 推理模型要求后续请求中 AiMessage 必须包含 thinking 字段
                // 如果原始消息没有 thinking，注入占位符
                if (oConvertUtils.isEmpty(aiMessage.thinking())) {
            ...
            // 工具执行结果：必须在 AiMessage(tool_calls) 之后保存
    ...
    // update-begin---author:Claude ---date:2026-05-09---for:【AI路况分析】添加工具调用状态跟踪，防止循环调用-----------
    /**
     * 构建工具调用状态提示词
     * 分析历史消息，统计每个工具已被调用的次数，生成状态提示词指导模型下一步操作
     *
     * @param messages 消息历史
     * @return 工具调用状态提示词，如果没有工具调用历史则返回null
     */
    private String buildToolCallStatusMessage(List<ChatMessage> messages) {
        // 统计各工具调用次数
        int mapsGeoCount = 0;
        int mapsDirectionCount = 0;
        String lastOriginLocation = null;
        String lastDestinationLocation = null;
        ...
        for (ChatMessage msg : messages) {
            if (msg instanceof ToolExecutionResultMessage) {
                ...
                // 处理工具名可能为null或unknown的情况
                if (oConvertUtils.isEmpty(toolName) || "unknown".equals(toolName)) {
                    // 尝试从结果内容推断工具类型
                    if (oConvertUtils.isNotEmpty(result)) {
                        if (result.contains("\"location\"")) {
                            toolName = "maps_geo";
                        } else if (result.contains("route") || result.contains("path") || result.contains("distance")) {
                ...
                if (oConvertUtils.isEmpty(toolName)) {
                    continue; // 跳过无法识别的工具调用
                }

                if ("maps_geo".equals(toolName)) {
                    mapsGeoCount++;
                    // 尝试提取坐标
                    try {
                        JSONObject resultJson = JSONObject.parseObject(result);
                        if (resultJson.containsKey("results") && !resultJson.getJSONArray("results").isEmpty()) {
                            JSONObject firstResult = resultJson.getJSONArray("results").getJSONObject(0);
                            if (firstResult.containsKey("location")) {
                                String location = firstResult.getString("location");
                                // 根据调用次数判断是起点还是终点
                                if (mapsGeoCount == 1) {
                                    lastOriginLocation = location;
                                } else if (mapsGeoCount == 2) {
                                    lastDestinationLocation = location;
                                }
                            }
                        }
                    } catch (Exception e) {
                ...
                } else if ("maps_direction_driving".equals(toolName)) {
                    mapsDirectionCount++;
                }
            }
        }
        ...
        statusBuilder.append("## 🔴 工具调用状态跟踪（由系统自动控制）\n\n");
        statusBuilder.append("**已完成的操作：**\n");
        statusBuilder.append("- maps_geo 已调用次数：").append(mapsGeoCount).append("/2（最多2次）\n");
        if (mapsGeoCount >= 1 && oConvertUtils.isNotEmpty(lastOriginLocation)) {
            statusBuilder.append("  ✅ 起点坐标：").append(lastOriginLocation).append("\n");
        }
        if (mapsGeoCount >= 2 && oConvertUtils.isNotEmpty(lastDestinationLocation)) {
            statusBuilder.append("  ✅ 终点坐标：").append(lastDestinationLocation).append("\n");
        }
        statusBuilder.append("- maps_direction_driving 已调用次数：").append(mapsDirectionCount).append("/1（最多1次）\n\n");

        // 根据状态给出下一步指令
        if (mapsGeoCount == 0) {
            statusBuilder.append("**下一步指令：** 请先调用 maps_geo 查询起点坐标（北京市朝阳区奥林佳泰大厦）\n");
        } else if (mapsGeoCount == 1) {
            statusBuilder.append("**下一步指令：** 请立即调用 maps_geo 查询终点坐标（北京市海淀区西三旗育新花园）\n");
            statusBuilder.append("⚠️ 注意：必须使用完整地址\"北京市海淀区西三旗育新花园\"，避免定位到大兴区\n");
        } else if (mapsGeoCount >= 2 && mapsDirectionCount == 0) {
            statusBuilder.append("**下一步指令：** 两个坐标已获取，请**立即**调用 maps_direction_driving 规划路线\n");
            statusBuilder.append("参数：origin=\"").append(lastOriginLocation != null ? lastOriginLocation : "起点坐标")
                         .append("\", destination=\"").append(lastDestinationLocation != null ? lastDestinationLocation : "终点坐标").append("\"\n");
            statusBuilder.append("🚫 **禁止再次调用 maps_geo！**\n");
        } else if (mapsDirectionCount >= 1) {
            statusBuilder.append("**状态：** 路线规划已完成。请整合结果输出路况分析。\n");
            statusBuilder.append("🚫 **所有工具调用已完成，禁止再次调用任何工具！**\n");
        }

        return statusBuilder.toString();
    }
    // update-end---author:Claude ---date:2026-05-09---for：【AI路况分析】添加工具调用状态跟踪，防止循环调用-----------
}
