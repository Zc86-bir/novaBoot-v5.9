    ...
    public Result<?> test(@RequestBody AiragModel airagModel) {
        ...
        try {
            if(LLMConsts.MODEL_TYPE_LLM.equals(airagModel.getModelType())){
                ...
                //update-begin---author:Zc86-bir ---date:2026-05-03  for：【issues/9595】自定义供应商Embedding模型测试-----------
                EmbeddingModel embeddingModel;
                if (LLMConsts.MODEL_PROVIDER_CUSTOM.equals(airagModel.getProvider())) {
                    embeddingModel = CustomModelFactory.createEmbeddingModel(aiModelOptions);
                } else {
                    embeddingModel = AiModelFactory.createEmbeddingModel(aiModelOptions);
                }
                //update-end---author:Zc86-bir ---date:2026-05-03  for：【issues/9595】自定义供应商Embedding模型测试-----------
