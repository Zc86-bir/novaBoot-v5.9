...
public class EmbeddingHandler implements IEmbeddingHandler {
    ...
    public Map<String, Object> embeddingDocument(String knowId, AiragKnowledgeDoc doc) {
        ...
        EmbeddingModel embeddingModel = createEmbeddingModel(model, modelOp);
    ...
    public QueryRouter getQueryRouter(List<String> knowIds, Integer topNumber, Double similarity) {
        ...
        for (String knowId : knowIds) {
            ...
            EmbeddingModel embeddingModel = createEmbeddingModel(model, modelOptions);
    ...
    public static AiModelOptions buildModelOptions(AiragModel model) {
        ...
        if (oConvertUtils.isObjectNotEmpty(model.getCredential())) {
            ...
            //update-begin---author:Zc86-bir ---date:2026-05-03  for：【AI模型配置优化】向量模型支持自定义API参数-----------
            // 将协议类型字段存入 extraParams
            ...
            if (!extraParams.isEmpty()) {
                modelOpBuilder.extraParams(extraParams);
            }
            //update-end---author:Zc86-bir ---date:2026-05-03  for：【AI模型配置优化】向量模型支持自定义API参数-----------
        }
    ...
    /**
     * 创建向量模型：CUSTOM provider 路由到 CustomModelFactory，否则使用 AiModelFactory
     */
    private EmbeddingModel createEmbeddingModel(AiragModel model, AiModelOptions options) {
        if (LLMConsts.MODEL_PROVIDER_CUSTOM.equalsIgnoreCase(model.getProvider())) {
            return CustomModelFactory.createEmbeddingModel(options);
        }
        return AiModelFactory.createEmbeddingModel(options);
    }
